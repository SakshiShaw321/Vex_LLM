# Vex_LLM — Build a Vector Database from Scratch in Python
...
```
*(~1.3 GB — default generation model; use `llama3.2` for higher quality)*
...
```powershell
git clone https://github.com/SakshiShaw321/Vex_LLM.git
cd Vex_LLM
```
...
4. Long documents are automatically split into overlapping 400-word chunks
...
| LLM answer is slow | Normal on CPU. Default is `llama3.2:1b`; try `llama3.2` for better quality |

### Use a Different LLM

To switch models, edit `gen_model` in [main.py](main.py):

```python
self.gen_model = "llama3.2"   # higher quality, slower
```
